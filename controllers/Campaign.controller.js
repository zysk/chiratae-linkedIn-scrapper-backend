...
import { initializeServices } from "../services/AppService";
import { CalculateRating } from "../helpers/linkedin/CalculateRating";
import { rolesObj } from "../helpers/utils/Constants";
import { sendCustomMailToSavanta, sendMail } from "../helpers/email/nodeMailer";
...
import { randomBoolean, randomIntFromInterval } from "../helpers/utils/utils";
...
export const handleLogoutAndLoginAnotherAccount = async (req, res, next) => {
    try {
        const { driver: maindriver, cronFunc } = await initializeServices();
        maindriver.sleep(1000);

        let logoutButton = await maindriver.wait(until.elementsLocated(By.xpath(`(//span[@class='t-12 global-nav__primary-link-text'])[1]`)));
        if (logoutButton) {
            await maindriver.findElement(By.xpath(`(//span[@class='t-12 global-nav__primary-link-text'])[1]`)).click();

            maindriver.sleep(1000);

            let signOutButtonLocate = await maindriver.wait(until.elementsLocated(By.linkText("Sign Out")));
            if (signOutButtonLocate) {
                let signOutButton = await maindriver.findElement(By.linkText("Sign Out")).click();
                try {
                    let signOutButtonModal = await maindriver.wait(until.elementsLocated(By.xpath(`//button[@class="full-width mt4 artdeco-button artdeco-button--muted artdeco-button--2 artdeco-button--secondary ember-view"]`)), 8000);
                    if (signOutButtonModal) {
                        maindriver.findElement(By.xpath(`//button[@class="full-width mt4 artdeco-button artdeco-button--muted artdeco-button--2 artdeco-button--secondary ember-view"]`)).click();
                        let url = await maindriver.getCurrentUrl();
            ...
            // await maindriver.wait(until.elementsLocated(By.xpath(``))).click()
        }
    } catch (error) {
        console.error(error);
    }
...
export const checkLinkedInLoginFunc = async () => {
    const { driver: maindriver, cronFunc } = await initializeServices();
    await maindriver.get("https://www.linkedin.com/checkpoint/lg/sign-in-another-account");
    let isLogin = false;
    let url = await maindriver.getCurrentUrl();
...
export const linkedInLogin = async (req, res, next) => {
    ...
    /**
     * Scrapes detailed profile information from LinkedIn for users marked for scraping
     * @param {RedisClient} redisClientParam - Redis client for managing scraping state
     * @returns {Promise<boolean>} - Returns true if scraping completed, false if login failed
     *
     * Business Logic:
     * 1. Sets busy flag in Redis
     * 2. Verifies LinkedIn login status
     * 3. Fetches up to 50 unscrapped user profiles
     * 4. For each profile:
     *    - Scrapes current position
     *    - Scrapes location
     *    - Scrapes contact information
     *    - Updates user record with scraped data
     * 5. Handles rate limiting through random delays
     */
    export const linkedInProfileScrapping = async (redisClientParam) => {
        ...
        if (!loggedIn) {
            const allEmails = await LinkedInAccountsModel.find().exec();
            const emails = allEmails.map(element => element.name);
            await sendMail(emails);
            await redisClientParam.set("isFree", "true");
            return false;
        }

        // Get users pending scraping
        const userArr = await User.find({
            role: rolesObj?.CLIENT,
            searchCompleted: false
        }).limit(50).lean().exec();

        if (!userArr.length) return true;
        ...
        for (let j = 0; j < userArr.length; j++) {
            try {
                const campaignObj = await Campaign.findById(userArr[j].campaignId).exec();
                await driver.get(`${userArr[j].link}`);

                // Random delay and scroll to avoid detection
                await driver.sleep(randomIntFromInterval(1000, 15000));
                if (randomBoolean()) {
                    await driver.executeScript(
                        `window.scrollTo(0, ${randomIntFromInterval(100, 1000)})`
                    );
                }
    ...
    /**
     * Manages LinkedIn search campaigns and result processing
     */
    export class CampaignController {
        /**
         * Creates a new LinkedIn search campaign
         * @param {Request} req Express request with campaign parameters
         * @param {Response} res Express response
         *
         * Campaign Parameters:
         * - searchQuery: Main search keywords
         * - company: Target company filters
         * - school: Education filters
         * - scheduleDate: When to run the campaign
         */
        ...
        /**
         * Processes search results for a campaign
         * @param {string} campaignId Campaign identifier
         *
         * Business Logic:
         * 1. Fetches campaign details
         * 2. Performs LinkedIn search
         * 3. Processes and saves results
         * 4. Updates campaign status
         */
        static async processCampaignResults(campaignId) {
            const campaign = await Campaign.findById(campaignId);
            if (!campaign) throw new Error("Campaign not found");

            // Update campaign status
            await Campaign.findByIdAndUpdate(campaignId, {
                status: generalModelStatuses.PROCESSING
            });

            try {
                const results = await searchLinkedInFn(campaign);
                await this.saveResults(campaign, results);

                await Campaign.findByIdAndUpdate(campaignId, {
                    status: generalModelStatuses.COMPLETED,
                    completedAt: new Date()
                });
            } catch (error) {
                await Campaign.findByIdAndUpdate(campaignId, {
                    status: generalModelStatuses.FAILED,
                    error: error.message
                });
                throw error;
            }
