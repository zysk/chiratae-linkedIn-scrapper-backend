...
import { ErrorMessages, SuccessMessages, leadStatusObj, ratingObj, Role } from '../helpers/Constants';
...
export const getLeads = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (campaignId && mongoose.Types.ObjectId.isValid(campaignId as string)) {
        ...
        const campaign = await Campaign.findOne({ _id: campaignId, createdBy: req.user?.id });
        if (!campaign) {
        ...
        const userCampaigns = await Campaign.find({ createdBy: req.user?.id }).select('_id');
        query.campaignId = { $in: userCampaigns.map(c => c._id) };
    }

    // Filter by assigned user if provided and valid
    if (assignedToId && mongoose.Types.ObjectId.isValid(assignedToId as string)) {
      query.leadAssignedToId = assignedToId;
    }

    // Filter by status if provided and valid
    if (status && Object.values(leadStatusObj).includes(status as typeof leadStatusObj[keyof typeof leadStatusObj])) {
      query.status = status;
    }

    // Filter by rating if provided and valid
    if (rating && Object.values(ratingObj).includes(rating as typeof ratingObj[keyof typeof ratingObj])) {
      query.rating = rating;
    }
    ...
    res.status(200).json(paginatedResponse('Leads retrieved', leads, {
      total,
      page,
      limit,
...
export const getLeadById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    if (lead.campaignId && (lead.campaignId as any).createdBy?.toString() !== req.user?.id) {
 ...
 */
export const createManualLead = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const { clientId, campaignId, status, rating, profileData, leadAssignedToId } = req.body;

  if (!clientId || !campaignId) {
    return next(badRequest('Client ID (profile URL/ID) and Campaign ID are required'));
  }
...
export const updateLead = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  if (leadAssignedToId !== undefined) { // Allow unassignment by passing null
      if (leadAssignedToId !== null && !mongoose.Types.ObjectId.isValid(leadAssignedToId)){
  ...
  try {
    ...
    if (updateData.status && updateData.status !== lead.status) {
      ...
      sendLeadStatusChangeEmail(id, lead.status, updateData.status)
        .catch(err => console.error('Failed to send status change email:', err));
    }

    if (updateData.leadAssignedToId !== undefined &&
      ...
      await createLeadLogEntry(
        ...
        updateData.leadAssignedToId ? updateData.leadAssignedToId.toString() : 'None'
      );
    }
    ...
    res.status(200).json(dataResponse(SuccessMessages.LEAD_UPDATED, updatedLead));
