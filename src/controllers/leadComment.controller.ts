...
import { createLeadLogEntry } from "./leadLog.controller"; // Import log creation function
import { sendNewCommentEmail } from "../services/email.service"; // Import email service
...
export const addLeadComment = async (
  ...
  try {
    ...
    // Create log entry for comment added
    await createLeadLogEntry(
      leadId,
      "COMMENT_ADDED",
      `Comment added: ${comment.substring(0, 30)}${comment.length > 30 ? "..." : ""}`,
      userId,
    );

    // Send email notification
    sendNewCommentEmail(newComment._id).catch((err) =>
      console.error("Failed to send comment notification email:", err),
    );

    res
      .status(201)
      .json(dataResponse(SuccessMessages.COMMENT_ADDED, newComment));
  } catch (error: any) {
...
export const getLeadComments = async (
  ...
  try {
    ...
    res.status(200).json(
      paginatedResponse("Comments retrieved", comments, {
        total,
        page,
        limit,
...
export const getCommentById = async (
  ...
  try {
    const comment = await LeadComment.findById(id).populate(
      "userId",
...
export const updateLeadComment = async (
  ...
  if (
    ...
    return next(
      badRequest("Valid Comment ID and new comment text are required"),
    );
  }
  ...
  try {
    ...
    // Create log entry for comment updated
    await createLeadLogEntry(
      comment.leadId,
      "COMMENT_UPDATED",
      `Comment updated`,
      userId,
      oldComment.substring(0, 30) + (oldComment.length > 30 ? "..." : ""),
      newCommentText.substring(0, 30) +
        (newCommentText.length > 30 ? "..." : ""),
    );

    await comment.populate("userId", "firstName lastName email");
    res
      .status(200)
      .json(dataResponse(SuccessMessages.COMMENT_UPDATED, comment));
  } catch (error: any) {
    if (error.name === "ValidationError") {
      return next(badRequest(error.message));
    }
...
export const deleteLeadComment = async (
  ...
  try {
    ...
    await createLeadLogEntry(
      comment.leadId,
      "COMMENT_DELETED",
      `Comment deleted: ${comment.comment.substring(0, 30)}${comment.comment.length > 30 ? "..." : ""}`,
      userId,
    );
