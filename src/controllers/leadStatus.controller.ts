 ...
 */
export const createLeadStatus = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    res.status(201).json(dataResponse(SuccessMessages.LEAD_STATUS_CREATED, newStatus));
  } catch (error: any) {
    if (error.name === 'ValidationError') {
      return next(badRequest(error.message));
    }
    if (error.code === 11000) { // Handle duplicate name
 ...
 */
export const getAllLeadStatuses = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
 ...
 */
export const getLeadStatusById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const { id } = req.params;
  ...
  try {
    const status = await LeadStatusDef.findById(id)
      .populate('createdBy', 'firstName lastName');
...
export const updateLeadStatus = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    res.status(200).json(dataResponse(SuccessMessages.LEAD_STATUS_UPDATED, updatedStatus));
  } catch (error: any) {
    if (error.name === 'ValidationError') {
      return next(badRequest(error.message));
    }
 ...
 */
export const deleteLeadStatus = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const { id } = req.params;
  ...
  try {
    // TODO: Check if this status is in use by any leads before deletion
