 ...
 */
export const getLeadLogs = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
        ...
        res.status(200).json(paginatedResponse('Logs retrieved', logs, {
            total,
            page,
            limit,
...
export const getLogById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
        const logEntry = await LeadLog.findById(id).populate('userId', 'firstName lastName email');
        if (!logEntry) {
...
export const createLeadLogEntry = async (
    ...
    try {
        ...
        console.error(`Failed to create lead log entry for lead ${leadId}, action ${action}:`, error);
