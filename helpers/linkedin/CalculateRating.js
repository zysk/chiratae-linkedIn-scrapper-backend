...
import { matchWithSpecialCharacters } from "../regex/regexHelpers";
...
export const CalculateRating = (obj) => {
    try {
        ...
        // }
        if (
            obj.educationArr.some(
                (el) =>
                    new RegExp(matchWithSpecialCharacters("IIT"), "i").test(el.schoolName) ||
            ...
            if (obj?.experienceArr && obj?.experienceArr.length > 0 && obj?.experienceArr[0]?.year) {
                ...
                if (yearArr.length > 0 && yearArr.length == 4) {
                    if (
                        (new RegExp(matchWithSpecialCharacters("founder"), "i").test(obj.experienceArr[0].company) || new RegExp(matchWithSpecialCharacters("founder"), "i").test(obj.experienceArr[0].companyDetail)) &&
                        (yearArr.some((elx) => new RegExp(matchWithSpecialCharacters("mo"), "i").test(elx)) ||
                            (yearArr.some((elx) => new RegExp(matchWithSpecialCharacters("yr"), "i").test(elx)) && yearArr && parseInt(yearArr[0]) <= 1 && parseInt(yearArr[2]) <= 6))
                    ) {
                        return "MEDIUM";
                    } else {
                        return "LOW";
                    ...
                    return "MEDIUM";
                } else if (yearArr[1].includes("yr") && parseInt(yearArr[0]) <= 1) {
                    return "MEDIUM";
                } else {
                    return "LOW";
                }
