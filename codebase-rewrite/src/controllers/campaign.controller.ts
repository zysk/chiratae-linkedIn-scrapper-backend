...
import { successResponse, dataResponse, paginatedResponse, errorResponse } from '../interfaces/ApiResponse';
import { ErrorMessages, SuccessMessages, campaignStatusObj } from '../helpers/Constants';
...
export const createCampaign = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const { name, searchQuery, linkedInAccountId, proxyId, school, company, pastCompany, schedule } = req.body;

  if (!name || !searchQuery || !linkedInAccountId) {
    return next(badRequest('Campaign name, search query, and LinkedIn account ID are required'));
  }

  if (linkedInAccountId && !mongoose.Types.ObjectId.isValid(linkedInAccountId)) {
  ...
  try {
    ...
    res.status(201).json(dataResponse(SuccessMessages.CAMPAIGN_CREATED, newCampaign));
...
export const getAllCampaigns = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    res.status(200).json(paginatedResponse('Campaigns retrieved', campaigns, {
      total,
      page,
      limit,
...
export const getCampaignById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    const campaign = await Campaign.findOne({ _id: id, createdBy: req.user?.id }) // Ensure user owns campaign
...
export const updateCampaign = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  if (updateData.linkedInAccountId && !mongoose.Types.ObjectId.isValid(updateData.linkedInAccountId)) {
     return next(badRequest('Invalid LinkedIn Account ID format'));
  }
  if (updateData.proxyId && !mongoose.Types.ObjectId.isValid(updateData.proxyId)) {
     return next(badRequest('Invalid Proxy ID format'));
  }
  ...
  try {
    ...
    if (updateData.linkedInAccountId) {
      const account = await LinkedInAccount.findById(updateData.linkedInAccountId);
      if (!account) {
        return next(notFound(ErrorMessages.LINKEDIN_ACCOUNT_NOT_FOUND));
      }
    }
    ...
    res.status(200).json(dataResponse(SuccessMessages.CAMPAIGN_UPDATED, updatedCampaign));
  } catch (error: any) {
    if (error.name === 'ValidationError') {
      return next(badRequest(error.message));
    }
...
export const deleteCampaign = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    const deletedCampaign = await Campaign.findOneAndRemove({ _id: id, createdBy: req.user?.id });
    if (!deletedCampaign) {
      return next(notFound(ErrorMessages.CAMPAIGN_NOT_FOUND));
    }
...
export const startCampaign = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  if (!initialDriver) { // Or get a driver from a pool if applicable
  ...
  try {
    ...
    performSearch(initialDriver, campaign).catch(err => {
        ...
        console.error(`Background search failed for campaign ${campaign._id}:`, err);
    });
 ...
 */
export const stopCampaign = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const { id } = req.params;
 ...
 */
export const linkedInLogin = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (!initialDriver) { // Check if the shared driver is available
         return next(serverError('WebDriver not initialized. Cannot perform login.'));
    }

    try {
        // Note: This uses the shared driver. For concurrent logins, a pool or separate drivers might be needed.
        const result: LoginResult = await performLinkedInLogin(initialDriver, linkedInAccountId);
 ...
 */
export const checkLinkedInLogin = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    if (!initialDriver) {
         return next(serverError('WebDriver not initialized.'));
    }
    try {
        const isLoggedIn = await checkLinkedInLoginStatus(initialDriver);
        res.status(200).json(dataResponse(isLoggedIn ? 'Logged In' : 'Not Logged In', { isLoggedIn }));
    } catch (error: any) {
        next(serverError(error.message));
    }
};
 ...
 */
export const sendLinkedInCaptchaInput = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
        ...
        if (success) {
            ...
            const isLoggedIn = await checkLinkedInLoginStatus(initialDriver);
             res.status(200).json(dataResponse('CAPTCHA submitted. Re-check login status.', { submitted: true, isLoggedIn }));
        } else {
    ...
    try {
        const success = await submitOtpCode(initialDriver, otpCode);
         if (success) {
            // After submitting, immediately check status again
            const isLoggedIn = await checkLinkedInLoginStatus(initialDriver);
            res.status(200).json(dataResponse('OTP submitted. Re-check login status.', { submitted: true, isLoggedIn }));
        } else {
